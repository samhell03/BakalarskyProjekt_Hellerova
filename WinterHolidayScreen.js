...
import { getItemsForTrip, insertItem, deleteItem, createItemsTable, insertWinterItems, deleteAllItemsForTrip } from './database'; // Upravíme import

export default function WinterHolidayScreen({ route }) {
    ...
    const loadItems = async () => {
      try {
        ...
        if (savedItems.length === 0) {
          await insertWinterItems(trip.id);
  ...
  const loadDefaultWinterItems = async () => {
    try {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.refreshContainer}>
        <TouchableOpacity onPress={loadDefaultWinterItems}>
