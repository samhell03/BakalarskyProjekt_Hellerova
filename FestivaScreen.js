...
import { getItemsForTrip, insertItem, deleteItem, createItemsTable, insertFestivalItems, deleteAllItemsForTrip } from './database';
export default function FestivalScreen({ route }) {
    ...
    const loadItems = async () => {
      try {
        ...
        if (savedItems.length === 0) {
          await insertFestivalItems(trip.id);
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subHeader}>
        Chystáte se na <Text style={styles.strong}>festival/koncert</Text>, pojďme si společně zabalit!
      </Text>
