...
import { getItemsForTrip, insertItem, deleteItem, createItemsTable, insertCampingItems, deleteAllItemsForTrip } from './database'; // Upravíme import
...
export default function CampingScreen({ route }) {
    ...
    const loadItems = async () => {
      try {
        ...
        if (savedItems.length === 0) {
          await insertCampingItems(trip.id);
  ...
  const loadDefaultCampingItems = async () => {
    try {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subHeader}>
        Chystáte se <Text style={styles.strong}>kempovat</Text>, pojďme si společně zabalit!
      </Text>

      {/* Refresh tlačítko */}
      <View style={styles.refreshContainer}>
        <TouchableOpacity onPress={loadDefaultCampingItems}>
